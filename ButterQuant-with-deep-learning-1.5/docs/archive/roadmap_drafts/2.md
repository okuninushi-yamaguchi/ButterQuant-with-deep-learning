# 阶段3：深度学习集成方案

## 🎯 设计目标

1. **本地训练**：利用你的 RTX 4060 训练模型
2. **云端推理**：部署轻量级模型到服务器
3. **数据准备**：利用现有的 `market_research.db` 数据
4. **增量学习**：每周更新模型（无需重新训练）

---

## 📊 数据流架构

```
┌────────────────────────────────────────────┐
│      每日扫描（daily_scanner.py）         │
│  - 扫描 Nasdaq 100 + S&P 500               │
│  - 写入 market_research.db                 │
└──────────────┬─────────────────────────────┘
               │ 每天新增 23MB
               ▼
┌────────────────────────────────────────────┐
│      TimescaleDB (PostgreSQL)              │
│  - 时序优化存储                            │
│  - 自动数据压缩（历史数据）               │
│  - 连续聚合（预计算特征）                 │
└──────────────┬─────────────────────────────┘
               │ SQL 查询
               ▼
┌────────────────────────────────────────────┐
│      本地训练环境（你的电脑）             │
│  ├─ 数据导出：PostgreSQL → Parquet        │
│  ├─ 特征工程：Pandas + NumPy              │
│  ├─ 模型训练：PyTorch (RTX 4060)          │
│  └─ 模型导出：ONNX / TorchScript          │
└──────────────┬─────────────────────────────┘
               │ 上传模型
               ▼
┌────────────────────────────────────────────┐
│      云端推理服务                          │
│  - ONNX Runtime (CPU推理)                  │
│  - 集成到 Flask API                        │
│  - 每周自动更新模型                        │
└────────────────────────────────────────────┘
```

---

## 🔬 数据准备

### **1. 数据库迁移到 TimescaleDB**
TimescaleDB 是 PostgreSQL 的时序数据库扩展，特别适合金融数据：

```sql
-- 创建超表（Hypertable）
CREATE TABLE daily_metrics (
    time TIMESTAMPTZ NOT NULL,
    ticker TEXT NOT NULL,
    -- ... 其他字段（与你的 SQLite 相同）
    PRIMARY KEY (time, ticker)
);

SELECT create_hypertable('daily_metrics', 'time');

-- 自动压缩历史数据（节省 50-70% 空间）
ALTER TABLE daily_metrics SET (
  timescaledb.compress,
  timescaledb.compress_segmentby = 'ticker'
);

SELECT add_compression_policy('daily_metrics', INTERVAL '7 days');
```

**优势**：
- ✅ 查询速度比 SQLite 快 **10-100倍**
- ✅ 自动分区（按时间）
- ✅ 支持连续聚合（预计算特征）

---

### **2. 数据导出脚本**
```python
# scripts/export_training_data.py
import pandas as pd
from sqlalchemy import create_engine

engine = create_engine('postgresql://user:pass@localhost/butterquant')

# 导出最近 1 年的数据用于训练
query = """
SELECT 
    time, ticker, current_price, 
    trend_direction, predicted_vol, 
    delta, gamma, vega, theta,
    total_score, recommendation,
    -- 计算标签（7天后的实际收益）
    LEAD(current_price, 7) OVER (PARTITION BY ticker ORDER BY time) 
        / current_price - 1 AS future_return_7d
FROM daily_metrics
WHERE time >= NOW() - INTERVAL '1 year'
  AND total_score > 50  -- 只用高质量数据训练
ORDER BY time
"""

df = pd.read_sql(query, engine)

# 保存为 Parquet（压缩率高 + 读取快）
df.to_parquet('training_data.parquet', compression='snappy')
print(f"✅ Exported {len(df)} records")
```

---

## 🧠 模型设计

### **预测目标选择**

根据你的量化策略，我建议 **3个预测任务**：

#### **任务 1：策略成功率预测（推荐优先做）**
- **输入**：当前 Greeks + 波动率 + 傅立叶特征
- **输出**：7天后策略是否盈利（二分类）
- **用途**：过滤掉低概率机会

```python
# 数据标签
df['label'] = (df['future_return_7d'].abs() < 0.02).astype(int)  # 横盘 = 1
```

#### **任务 2：波动率预测（中期）**
- **输入**：历史波动率序列 + GARCH 预测
- **输出**：未来 7 天的实际波动率
- **用途**：检测 GARCH 模型误差，优化 IV 错误定价

#### **任务 3：价格方向预测（长期）**
- **输入**：多因子特征
- **输出**：上涨/横盘/下跌（三分类）
- **用途**：选择 Call/Put/Iron 蝴蝶

---

### **模型架构（PyTorch）**

```python
# models/strategy_predictor.py
import torch
import torch.nn as nn

class ButterflyNet(nn.Module):
    """蝴蝶策略预测模型"""
    
    def __init__(self, input_dim=20, hidden_dim=128):
        super().__init__()
        
        # 特征提取
        self.feature_net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU(),
            nn.Dropout(0.3),
            
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.BatchNorm1d(hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.2),
        )
        
        # 分类头
        self.classifier = nn.Linear(hidden_dim // 2, 2)  # 盈利/亏损
        
    def forward(self, x):
        features = self.feature_net(x)
        logits = self.classifier(features)
        return logits

# 训练脚本
def train_model(train_loader, val_loader, epochs=50):
    model = ButterflyNet(input_dim=20).cuda()
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)
    criterion = nn.CrossEntropyLoss()
    
    best_val_acc = 0
    for epoch in range(epochs):
        model.train()
        for batch_x, batch_y in train_loader:
            batch_x, batch_y = batch_x.cuda(), batch_y.cuda()
            
            optimizer.zero_grad()
            logits = model(batch_x)
            loss = criterion(logits, batch_y)
            loss.backward()
            optimizer.step()
        
        # 验证
        val_acc = evaluate(model, val_loader)
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            torch.save(model.state_dict(), 'best_model.pth')
    
    return model
```

---

### **特征工程**

```python
# scripts/feature_engineering.py
import pandas as pd
import numpy as np

def engineer_features(df):
    """生成训练特征"""
    features = []
    
    # 1. Greeks 归一化
    features.extend([
        df['delta'] / df['current_price'],
        df['gamma'] / df['current_price']**2,
        df['vega'] / 100,  # IV 单位转换
        df['theta'] / df['current_price']
    ])
    
    # 2. 波动率特征
    features.extend([
        df['predicted_vol'],
        df['current_iv'],
        df['vol_mispricing'],
        df['iv_percentile']
    ])
    
    # 3. 傅立叶特征
    features.extend([
        (df['trend_direction'] == 'UPTREND').astype(int),
        df['trend_slope'],
        df['dominant_period']
    ])
    
    # 4. 时序特征（滚动统计）
    df['price_ma7'] = df.groupby('ticker')['current_price'].transform(
        lambda x: x.rolling(7).mean()
    )
    df['price_std7'] = df.groupby('ticker')['current_price'].transform(
        lambda x: x.rolling(7).std()
    )
    features.extend([
        df['current_price'] / df['price_ma7'],
        df['price_std7'] / df['price_ma7']
    ])
    
    # 5. 策略指标
    features.extend([
        df['profit_ratio'],
        df['prob_profit'],
        df['total_score'] / 100
    ])
    
    return pd.DataFrame(features).T
```

---

## 🚀 部署流程

### **1. 模型转换（ONNX）**
```python
# scripts/export_onnx.py
import torch
from models.strategy_predictor import ButterflyNet

model = ButterflyNet(input_dim=20)
model.load_state_dict(torch.load('best_model.pth'))
model.eval()

dummy_input = torch.randn(1, 20)
torch.onnx.export(
    model,
    dummy_input,
    'butterfly_model.onnx',
    input_names=['features'],
    output_names=['logits'],
    dynamic_axes={'features': {0: 'batch_size'}}
)
```

---

### **2. 集成到 Flask**
```python
# backend/ml_inference.py
import onnxruntime as ort
import numpy as np

class MLPredictor:
    def __init__(self, model_path='models/butterfly_model.onnx'):
        self.session = ort.InferenceSession(model_path)
        
    def predict_success(self, features):
        """预测策略成功概率"""
        # features: [delta, gamma, vega, theta, ...]
        input_array = np.array(features, dtype=np.float32).reshape(1, -1)
        
        outputs = self.session.run(None, {'features': input_array})
        logits = outputs[0]
        
        # Softmax
        probs = np.exp(logits) / np.exp(logits).sum()
        return probs[0][1]  # 盈利概率

# 在 analyzer.py 中调用
ml_predictor = MLPredictor()

def full_analysis(ticker):
    # ... 原有逻辑 ...
    
    # 提取特征
    features = [
        result['greeks']['delta'],
        result['greeks']['gamma'],
        # ... 其他特征
    ]
    
    # ML 预测
    ml_score = ml_predictor.predict_success(features)
    
    # 融合到总分
    result['ml_success_prob'] = ml_score
    result['total_score'] = result['total_score'] * 0.7 + ml_score * 100 * 0.3
    
    return result
```

---

## 📅 训练计划

### **初期（第1-2周）**
- [x] 导出 6个月的历史数据（约 4GB）
- [x] 特征工程 + 数据清洗
- [x] 训练基础模型（Binary Classification）

### **中期（第3-8周）**
- [ ] 超参数调优（学习率、Dropout、网络深度）
- [ ] 交叉验证（按时间分割）
- [ ] 模型集成（XGBoost + PyTorch 融合）

### **长期（3个月后）**
- [ ] Transformer 架构（处理时序依赖）
- [ ] 强化学习（端到端策略优化）
- [ ] AutoML（自动特征选择）

---

## 💰 成本估算

- **训练成本**：¥0（你的 RTX 4060，电费可忽略）
- **云端推理**：¥50/月（ONNX Runtime，CPU 足够）
- **数据存储**：¥100/月（PostgreSQL + TimescaleDB）

**总计**：¥150/月（增量成本）

---

## 🎯 预期效果

根据类似项目经验：
- **策略成功率预测**：准确率 60-70%（随机是 50%）
- **波动率预测**：RMSE 比 GARCH 降低 15-25%
- **总分提升**：高分机会的实际盈利率提高 10-20%

**关键**：深度学习不是万能药，它是对传统量化模型的**增强**，不是替代。